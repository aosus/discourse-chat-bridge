...
export default async function get_latest_posts_(client) {
    ...
    client.command('get_latest_posts', async (ctx) => {
        ...
        if (data.includes('itemprop="image"')) {
            ...
            let caption = `<b><a href='${process.env.url}/t/${get?.topic_slug}/${get?.topic_id}'>${get?.topic_title}</a></b> \n\n`;
            caption += `<b>الكاتب:</b> <a href='${process.env.url}/u/${get?.username}'>${get?.name}</a> \n`;
