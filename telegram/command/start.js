...
export default async function start(client, Markup) {
    ...
    client.start(async (ctx) => {
        ...
        let but_1 = [
            Markup.button.url(process.env.title_discourse, process.env.url)
        ];
        let button = Markup.inlineKeyboard([but_1]);
        let message = `<b>مرحبا بك ${name_from} في جسر ${process.env.title_discourse} 👋</b> \n\n`
        ...
        message += `▪ ربط حسابك على ${process.env.title_discourse} \n/discourse \n`
        message += '▪ تفعيل البوت \n/activation'
