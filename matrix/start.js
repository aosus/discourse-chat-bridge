...
export default async function start(roomId, sender, name, body, event, RichReply, client) {
    ...
    if (body === 'خدمة' || body === 'خدمه' || body === 'start' || body === '#') {
        ...
        let message = `<b>مرحبا بك ${name} في جسر ${process.env.title_discourse || config?.title_discourse} 👋</b> <br>`
        ...
        message += `▪ ربط حسابك على ${process.env.title_discourse || config?.title_discourse} <br>6- discourse <br><br>`
