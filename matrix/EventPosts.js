...
export default async function EventPosts_(client) {
    ...
    await EventPosts(async e => {
        ...
        try {
            ...
            let cooked = e?.cooked;
            let raw = e?.raw;
            let response = await fetch(config?.url + `/t/${topic_slug}/${topic_id}`, { method: 'GET' });
            ...
            if (data.includes('itemprop="image"')) {
                ...
                for (let item of getRoom?.array) {
                    ...
                    if (item?.categories === category_id) {
                        ...
                        let caption = `<b><a href='${config?.url}/t/${topic_slug}/${topic_id}'>${topic_title}</a></b> <br><br>`;
                        caption += `<b>الكاتب:</b> <a href='${config?.url}/u/${username}'>${name}</a> <br>`;
